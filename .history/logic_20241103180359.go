...
func (h *Hero) attack(e *Enemy) {
	...
	e.health -= damage
	fmt.Printf("%s did %d points of damage, the %s has %d points of health left", h.name, damage, e.class, e.health)
}
...
func (e *Enemy) attack(h *Hero) {
	...
	h.health -= damage
	fmt.Printf("The %s did %d points of damage, %s has %d points of health left", e.class, damage, h.name, h.health)
}
