...
func (h Hero) attack(e *Enemy, w string) {
	weapon := h.weapons[w]
	damage := rand.Intn(h.damage) + 1 + weapon
	e.health -= damage
	if e.health <= 0 {
		e.dead = true
	}
...
//	func (h Hero) chooseWeapon(weapon string) int16 {
...
func spawnPlayer() (Hero, *Hero) {
	...
	case WARLOCK:
		...
		hP.damage = 4
		hP.health = 6
		hP.weapons = map[string]int16{"musket": 6}
		hP.slowed = false
		hP.weakened = false
		hP.imbued = false
		hP.dead = false
...
func spawnEnemy() (Enemy, *Enemy) {
	...
	case INQUISITOR:
		...
		eP.damage = 4
		eP.health = 6
		eP.weapons = map[string]int16{"cat of nine": 3}
		eP.slowed = false
		eP.weakened = false
		eP.repelled = false
		eP.dead = false
	case HUNTER:
		...
		eP.damage = 6
		eP.health = 4
		eP.weapons = map[string]int16{"bow": 5}
		eP.slowed = false
		eP.weakened = false
		eP.repelled = false
		eP.dead = false
	case KNIGHT:
		...
		eP.damage = 8
		eP.health = 10
		eP.weapons = map[string]int16{"sword": 8}
		eP.slowed = false
		eP.weakened = false
		eP.repelled = false
		eP.dead = false
...
func (e Enemy) attack(h *Hero, w string) {
	weapon := e.weapons[w]
	damage := rand.Intn(e.damage) + 1 + weapon
	h.health -= damage
	if h.health <= 0 {
		h.dead = true
	}
...
// func (e Enemy) chooseWeapon(weapon string) int16 {
// 	modifier := e.weapons[weapon]
// 	return modifier
